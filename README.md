...
## 📂 Estructura de Archivos (App: tienda_app) – Tutorial 03 (DRF)

```
tienda_app/
├── api/                    # Tutorial 03: Capa REST
│   ├── serializers.py      # LibroSerializer, OrdenInputSerializer (Adapter/DTO)
│   └── views.py            # CompraAPIView (POST /api/v1/comprar/)
├── domain/
├── infra/
├── services.py
├── views.py
...
- La misma **Capa de Servicio** (`CompraService`) se usa desde las vistas HTML y desde la API (reutilización de lógica de negocio).
- Evidencia de entrega: ver **ENTREGA.md**.
